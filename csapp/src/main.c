...
static void TestAddFunctionCallAndComputation()
{
    ...
    // init state
    cr->reg.rax = 0x12340000;
    cr->reg.rbx = 0x0;
    cr->reg.rcx = 0x8000660;
    cr->reg.rdx = 0xabcd;
    cr->reg.rsi = 0x7ffffffee2f8;
    cr->reg.rdi = 0x1;
    cr->reg.rbp = 0x7ffffffee210;
    cr->reg.rsp = 0x7ffffffee1f0;

    write64bits_dram(va2pa(0x7ffffffee210, cr), 0x08000660, cr);    // rbp
    write64bits_dram(va2pa(0x7ffffffee208, cr), 0x0, cr);
    write64bits_dram(va2pa(0x7ffffffee200, cr), 0xabcd, cr);
    write64bits_dram(va2pa(0x7ffffffee1f8, cr), 0x12340000, cr);
    write64bits_dram(va2pa(0x7ffffffee1f0, cr), 0x08000660, cr);    // rsp
   ...
   // copy to physical memory
    ...
    printf("begin\n");
    int time = 0;
    while ((cr->rip <= 14 * 0x40 + 0x00400000) &&
    ...
    match = match && (cr->reg.rcx == 0x8000660);
    match = match && (cr->reg.rdx == 0x12340000);
    match = match && (cr->reg.rsi == 0xabcd);
    match = match && (cr->reg.rdi == 0x12340000);
    match = match && (cr->reg.rbp == 0x7ffffffee210);
    match = match && (cr->reg.rsp == 0x7ffffffee1f0);
    ...
    match = match && (read64bits_dram(va2pa(0x7ffffffee210, cr), cr) == 0x08000660); // rbp
    match = match && (read64bits_dram(va2pa(0x7ffffffee208, cr), cr) == 0x1234abcd);
    match = match && (read64bits_dram(va2pa(0x7ffffffee200, cr), cr) == 0xabcd);
    match = match && (read64bits_dram(va2pa(0x7ffffffee1f8, cr), cr) == 0x12340000);
    match = match && (read64bits_dram(va2pa(0x7ffffffee1f0, cr), cr) == 0x08000660); // rsp
