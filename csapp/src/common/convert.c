...
uint64_t string2uint_range(const char *str, int start, int end)
{
    ...
    for(int i = start; i <= end; ++i)
    {
        ...
        else if(state == 2)
        {
            if('0' <= c && c <= '9')
            {
                ...
                if(uv < pv) 
                {
                    printf("(uint64_t)%s overflow, can't convert\n", str);
                    goto fail;
                }
                continue;
            }
    ...
    if(sign_bit == 0) return uv;
    else 
    {
        if((uv & 0x8000000000000000) != 0)
        {
            printf("(int64_t)%s signed overflow, can't convert\n", str);
            exit(0);
        }
    ...
    fail:
    printf("type converter: <%s> can't be converted to integer\n", str);
    exit(0);
}