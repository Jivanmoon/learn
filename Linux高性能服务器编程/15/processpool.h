...
//进程池的构造函数。参数listenfd是监听socket，它必须在创建进程池之前被创建，否则子进程无法直接引用它，参数process_number指定进程池中子进程的数量
template<typename T>
...
//父进程中m_idx的值为-1，子进程中m_idx值大于等于0，我们据此判断接下来要运行的是父进程代码还是子进程代码
template<typename T>
...
void processpool<T>::run_child() {
    ...
    while(!m_stop) {
        ...
        for(int i = 0; i < number; ++i) {
            ...
            if((sockfd == pipefd) && (events[i].events & EPOLLIN)) {
                int client = 0;
                //从父子进程之间的管道读取数据，并将结果保存在client中，如果读取成功，则表示有新的客户连接到来
            ...
            //如果是其他可读数据，那么必然是客户连接请求到来，调用逻辑处理对象的process方法处理之
    ...
    users = NULL;
    close(pipefd);
    //close(m_listenfd); 我们将这句话注释掉，以提醒读者：应该有m_listenfd的创建者来关闭这个文件描述符，即所谓的“对象（比如一个文件描述符，又或者一段堆内存）由那个函数创建，就应该由那个函数销毁”
    close(m_epollfd);
}
            ...
            //下面处理父进程收到的信号
                ...
                if(ret < 0) {
                    ...
                    for(int i = 0; i < ret; ++i) {
                        ...
                        case SIGCHLD:
                            ...
                            while((pid = waitpid(-1, &stat, WNOHANG)) > 0) {
                                for(int i = 0; i < m_process_number; ++i) {
                                    //如果进程池中第i个子进程退出了，则主进程关闭响应的通信管道，并设置相应的m_pid为-1，以标识该子进程已经退出
                        ...
                        case SIGTERM:
                        case SIGINT:
                            //如果父进程接收到终止信号，那么就杀死所有子进程，并等待他们全部结束。当然通知子进程结束的更好方法是向父子进程的通道发送特殊数据，读者不妨自己实现之
