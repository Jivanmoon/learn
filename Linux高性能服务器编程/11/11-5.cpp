...
public:
    ...
    tw_timer *add_timer(int timeout) {
        ...
        int ticks = 0;
        //下面根据待插入定时器的超时值计算它将在时间轮转动多少个滴答后被触发，并将该滴答数存储于ticks中，如果待插入定时器的超时值小于时间轮的槽间隔SI，则将ticks向上折合为1，否则就将ticks向下折合为timeout/SI
        if(timeout < SI) {
            ticks = 1;
        }
        else {
        ...
        //创建新的定时器，它的时间轮转动rotation圈后被触发，且位于第ts个槽上
        tw_timer *timer = new tw_timer(rotation, ts);

        //如果ts个槽中尚未有任何定时器，则把新建的定时器插入其中，并将该定时器设置为该槽的头结点
        if(!slots[ts]) {
        ...
        while(tmp) {
            ...
            //否则，说明定时器已经到期，于是执行定时任务，然后删除该定时器
                ...
                if(tmp == slots[cur_slot]) {
                    printf("delete headr in cur_slot\n");
