...
int main(int argc, char *argv[]) {
    ...
    while(!stop_server) {
        ...
        for(int i = 0; i < number; ++i) {
            ...
            //处理信号
                ...
                if(ret == -1) {
                    ...
                    for(int i = 0; i < ret; ++i) {
                        ...
                        {
                        case SIGALRM:
                            //用timeout变量标记有定时任务需要处理，但不立即处理定时任务，这是因为定时任务的优先级不是很高，我们有限处理其他重要任务
                            timeout = true;
                            break;
                        case SIGTERM:
                            stop_server = true;
            ...
            //处理客户连接上接收到的数据
                ...
                if(ret < 0) {
                    ...
                    //如果某个客户连接上有数据可读，则我们需要调整连接对应的定时器，以延迟该连接被关闭的时间
                    if(timer) {
                        time_t cur = time(nullptr);
                        timer->expire = cur + 3 * TIMESLOT;
        ...
        //最后处理定时器事件，因为IO事件有更高的优先级，当然，这样做将导致定时任务不能精确的按照预期事件执行
        if(timeout) {
            timer_handler();
            timeout = false;
        }
