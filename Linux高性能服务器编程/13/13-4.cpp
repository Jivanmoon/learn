...
int shmfd;
char *share_mem = 0;
//客户连接数组，进程用客户连接的编号来索引这个数组，即可得到相关的客户连接数据
client_data *users = 0;
...
//子进程运行的函数，参数idx指出该子进程处理的客户连接编号，users是保存所有客户连接数据的数组，参数share_mem指出共享内存的起始地址
int run_child(int idx, client_data *users, char *share_mem)
{
    epoll_event events[MAX_EVENT_NUMBER];
    //子进程使用IO复用技术来同时监听两个文件描述符：客户连接socket、与父进程同步的管道文件描述符
    ...
    while (!stop_child)
    {
        ...
        for (int i = 0; i < number; ++i)
        {
            ...
            if ((sockfd == connfd) && (events[i].events & EPOLLIN))
            {
                ...
                //将客户数据督导对应的读缓存中，该读缓存是共享内存的一段，它开始于idx*BUFFER_SIZE处，长度为BUFFER_SIZE字节。因此，各个客户连接的读缓存是共享的
