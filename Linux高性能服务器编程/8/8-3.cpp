...
//服务器处理http请求的而结果：NO_REQUEST表示请求不完整，需要继续读取客户数据；GET_REQUEST表示获得了一个完整的
//的客户请求；BAD_REQUEST表示客户请求有语法错误；FORBIDDEN_REQUEST表示客户对资源没有足够的访问权限；INTERNAL_ERROR
//表示服务器内部错误；CLOSED_CONNECTION表示客户端已经关闭连接
enum HTTP_CODE {NO_REQUEST, GET_REQUEST, BAD_REQUEST, FORBIDDEN_REQUEST, INTERNAL_ERROR, CLOSED_CONNECTION};

//为了简化问题，我们没有给客户发送一个完整的http应答报文，而只是根据服务器的处理结果发送如下成功或失败的信息
static const char* szert[] = {"I get a correct result\n", "Something wrong\n"};

//从状态机，用于解析出一行内容
LINE_STATUS parse_line(char* buffer, int& checked_index, int& read_index) {
    char temp;
    //checked_index指向buffer（应用程序的读缓冲区）中当前正在分析的字节，read_index指向buffer中客户端的尾部的下一字节，buffer中
    //0~checked_index字节都已分析完毕，第checked_index~(read_index-1)字节由下面的循环挨个分析
...
int main(int argc, char* argv[]) {
    ...
    int ret = bind(listenfd, (sockaddr*)&address, sizeof(sockaddr));
    assert(ret != -1);
    ...
    int fd = accept(listenfd, (sockaddr*)&client, &client_addrlen);
    if(fd < 0) {
        printf("errno is: %d\n", errno);
    }
    else {
