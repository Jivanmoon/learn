...
3、const修饰指针变量，const放在之前，指针变量所指向变量的值不可改变，指针值可以改变；const放在之后，指针变量所指向变量的值可以改变，指针值不可以改变；const修饰引用，const放在&之前，不能修改引用所表示的变量的值；const放在&之后，const的作用被忽略，可以修改引用所表示的变量的值。
4、非常量指针在指针赋值后可以改变指针值；引用在初始化后不能再作为别的变量的别名。
5、sizeof运算符作用于指针变量得到指针变量自身大小；作用于引用，得到引用所指向的变量的大小。
6、指针可以有多级，引用只有一级。
7、指针可以为空，而引用不能必须绑定到一个对象上。
...
* 详见：[细说new与malloc的10点区别](https://www.cnblogs.com/qg-whz/p/5140930.html)
...
### **在C++中const的用法（定义，用途）**
* 详见：[C/C++ 基础中的基础： const 修饰符用法总结！](https://zhuanlan.zhihu.com/p/90720012)
![const用法](./fig/const%E7%94%A8%E6%B3%95.jpg)
### **C++中的static用法和意义**
...
## 3、C++特性
### **C++中的函数重载、重写、隐藏（重定义）的区别**
* 详见：[c++中的函数重载、函数重写、函数重定义 ](https://www.cnblogs.com/nbk-zyc/p/12356271.html)
* 函数重载是指函数名相同，参数列表不同的函数实现方法。它们的返回值可以不同，但返回值不可以作为区分不同重载函数的标志。
* 函数重写是指函数名相同，参数列表相同，只有方法体不相同的实现方法。一般用于子类继承父类时对父类虚方法的重写。
* 函数隐藏是指子类重新定义父类中有相同名称的函数 (不包括虚函数重写 ) 。
* 重载的特征：
...
* 注意：隐藏与覆盖的区别就在于如下两条：
    ...
    2. 如果派生类的函数与基类的函数同名，并且参数也相同，但是基类函数没有virual关键字（如果有virual关键字，则满足重写的条件）。此时基类的函数被隐藏。
* 关于同名覆盖的结论（归纳：基类与派生类中存在同名成员；--- 同名覆盖）
    * 子类将隐藏父类中的同名成员；
    * 父类中的同名成员依然存在于子类中；
    * 可以通过作用域分辨符(::)访问被隐藏的父类中的同名成员；
    * 不可以直接通过子类对象访问父类成员；
### **C ++内存管理（热门问题）**
...
### **函数重载的时候怎么解决同名冲突**
* 详见：[C++的函数重载](https://www.cnblogs.com/skynet/archive/2010/09/05/1818636.html)
...
* 详见：[C++ inline关键字详解](https://zhuanlan.zhihu.com/p/151995167)
* inline和宏使用时的注意事项：
